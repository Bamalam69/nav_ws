...
// }
	...
	rclcpp::Publisher<inertiallabs_interfaces::msg::SensorData>::SharedPtr publisher_sensor_data;
	rclcpp::Publisher<inertiallabs_interfaces::msg::InsData>::SharedPtr publisher_ins_data;
	rclcpp::Publisher<inertiallabs_interfaces::msg::GpsData>::SharedPtr publisher_gps_data;
	rclcpp::Publisher<inertiallabs_interfaces::msg::GnssData>::SharedPtr publisher_gnss_data;
	rclcpp::Publisher<inertiallabs_interfaces::msg::MarineData>::SharedPtr publisher_marine_data;
...
public:
	...
	//   publisher_gps_data(create_publisher<sensor_msgs::msg::NavSatFix>("/gps/data", 1))
	  publisher_sensor_data(create_publisher<inertiallabs_interfaces::msg::SensorData>(
	  	"/Inertial_labs/sensor_data", 1
	  )),
	  publisher_ins_data(create_publisher<inertiallabs_interfaces::msg::InsData>(
	  	"/Inertial_labs/ins_data", 1
	  )),
	  publisher_gps_data(create_publisher<inertiallabs_interfaces::msg::GpsData>(
	  	"/Inertial_labs/gps_data", 1
	  )),
	  publisher_gnss_data(create_publisher<inertiallabs_interfaces::msg::GnssData>(
	  	"/Inertial_labs/gnss_data", 1
	  )),
	  publisher_marine_data(create_publisher<inertiallabs_interfaces::msg::MarineData>(
	  	"/Inertial_labs/marine_data", 1
	  ))
	{
...
private:
	...
	// if (context->publisher_ins_data->get_subscription_count() > 0) {
		...
		msg_ins_data.pos_std.z = data->KFAltStd;
		msg_ins_data.heading_std = data->KFHdgStd;
		publisher_ins_data->publish(msg_ins_data);
	// }
	...
	// if (context->publisher_gnss_data->get_subscription_count() > 0) {
		...
		msg_gnss_data.heading_std = data->HeadingGNSSStd;
		msg_gnss_data.pitch_std = data->PitchGNSSStd;
		publisher_gnss_data->publish(msg_gnss_data);
	// }

	// if (context->publisher_marine_data->get_subscription_count() > 0) {
		msg_marine_data.header.stamp = timestamp;
		msg_marine_data.header.frame_id = imu_frame_id;
		msg_marine_data.heave = data->Heave;
		msg_marine_data.surge = data->Surge;
		msg_marine_data.sway = data->Sway;
		msg_marine_data.heave_velocity = data->Heave_velocity;
		...
		msg_marine_data.sway_velocity = data->Sway_velocity;
		msg_marine_data.significant_wave_height = data->significant_wave_height;
		publisher_marine_data->publish(msg_marine_data);
	// }
	}
	...
	// 	// q.setW(-q.getW()); // Invert the quaternion (for some reason)
	// 	// msg_sensor_data.orientation = tf2::toMsg(q);
	// 	// msg_sensor_data.orientation_covariance[0] = 0.01;
	// 	msg_sensor_data.orientation_covariance[0] = -1;
	...
	// 	msg_sensor_data.angular_velocity_covariance[0] = 0.0005;
	...
	// 	msg_sensor_data.linear_acceleration_covariance[0] = 0.1000;
	...
	// 	// For shits and gigs:
	// 	msg_odo_data.child_frame_id = imu_frame_id;

	// 	// Linear velocities: (in ENU frame)
	...
	// 	msg_odo_data.twist.covariance[0] = 0.1;
	// 	msg_odo_data.twist.covariance[7] = 0.1;
	// 	msg_odo_data.twist.covariance[14] = 0.1;

	// 	// Orientation:
	// 	msg_odo_data.twist.twist.angular.x = data->Gyro[0] * deg2rad;
	// 	msg_odo_data.twist.twist.angular.y = data->Gyro[1] * deg2rad;
	// 	msg_odo_data.twist.twist.angular.z = data->Gyro[2] * deg2rad;

	// 	// Orientation covariance:
	// 	msg_odo_data.twist.covariance[21] = 0.01;
	// 	msg_odo_data.twist.covariance[28] = 0.01;
	// 	msg_odo_data.twist.covariance[35] = 0.01;
