...
namespace IL {
	...
	int Driver::connect(const char* url)
	{
		...
		RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "Opening %s...", pathStr.c_str());
		int result = port->open(pathStr.c_str());
		RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "Tried to open %s...", pathStr.c_str());
		...
		RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "HERE 1");

		sessionState = Off;
		workerThread = new std::thread(threadFunc, this);

		RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "HERE 2");

		readDevInfo();

		RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "HERE 3");

		if (sessionState < GotDevParams)
		{
			RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "sessionState < GotDevParams");
			disconnect();
			return 512;
		}

		RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "HERE 4");
